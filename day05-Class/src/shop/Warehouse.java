 ...
 * 
 * 제품들이 올려져 있거나 쌓여있는 창고의 선반은 각 창고마다 존재하기 때문에 구현은 멤버변수(실체변수)로 선언해야 한다.
 * -----------------------------------------------------------------
 * 
 * 메소드
 * 
 * set(Product product) : void 창고에 있는 제품 1개의 정보를 수정하는 기능
 *
 * get() : Product 창고에 있는 제품 1개의 정보를 출력하는 기능
 * 
 * add(Prodcut product) : void 창고에 신제품을 새로 들여놓는 기능
 * 
 * remove(Product product) : void 더이상 판매하지 않는 제품을 폐기하는 기능
 * 
 * getAllProducts() : Product[] 창고에 들어있는 전체 제품 목록을 가져오는 기능
 * 
 * 
 * @author Byun
 *
...
public class Warehouse {
	// 1. 멤버변수 선언부 : 제품을 쌓아둘 배열
	Product[] products;

	// 2. 생성자 선언부
	/**
	 * 기본 생성자 : 창고를 처음 지을 때 어떠게 지을 것인지 알려주는 생성자
	 */
	Warehouse() {
	...
	/*
	 * 객체지향적이 않다 Warehouse(int size){ this.products = new Product[size]; }
	 */
	...
	/**
	 * 파라미터로 전달된 정보와 일치하는 제품이 있는지 검색하여 (prodCode 필드가 같으면 같은 제품을 처리)
	 * 
	 * 일치하는 제품이 있으면 그 제품정보를 리턴, 없으면 null을 리턴
	 * 
	 * @param product
	 * @return
	 */
	...
	/**
	 * 매개변수로 넘겨진 제품정보와 일치하는 제품코드를 가진 제품에 대해 입력된 값으로 수정하여 저장
	 * 
	 * @param product
	 */
	...
	private Product findProduct(Product product) {
		...
		for (Product prod : products) {
			// 배열(선반)에 존재하는 제품정보(prod)가
			// 매개변수로 넘겨진 제품정보(product)와 일치하는지 비교 : prodCode만 비교
	...
	/**
	 * 매개변수로 입력된 제품이 배열(선반) 몇번째 위치에 존재하는지 그 인덱스를 찾는다
	 * 
	 * @param product
	 * @return
	 */
	private int findProductIdx(Product product) {
		...
		for (int idx = 0; idx < products.length; idx++) {
			...
			// products[idx]의 제품코드 필드(prodCode)가 매개변수로 넘어온 product의 제품코드 필드와 같은지 비교
